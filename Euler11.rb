...
$rows.each do |x|
	window = 0
	while window < ($column - 3)
		windowEnd = window + 3
		thisRow = x[window..windowEnd]
		product = thisRow.inject {|product, i| product * i}
		if(product > $Answer)
			$Answer = product
			puts x, window
		end
...
#Get the Columns
puts "Columns"
(0..$column-1).each do |x|
	window = 0
	while window < ($row - 3)
		windowEnd = window + 3
		product = 1
		(window..windowEnd).each do |y|
			product = product * $rows[y][x]
			if(product > $Answer)
				$Answer = product
				puts y, x
			end
...
for x in (0..($row-1))
	for y in (0..($column-1))
		...
		#L to R with two above
		xmin = x - 2
		ymin = y -2
		if(xmin < 0 or ymin < 0)
			next
		end
		if((xmin + 3) > ($row -1) || (ymin + 3) > ($column - 1))
			next
		end
		product = 1
		(0..3).each do |o|
			product = product * $rows[xmin + o][ymin + o]
		end
		if(product > $Answer)
			$Answer = product
			puts "One"
		...
		#R to L with two above
		xmin = x - 2
		ymin = y + 2
		if(xmin < 0 or ymin - 3 < 0)
			next
		end
		if((xmin + 3) > ($row -1) || ymin > ($column - 1))
			next
		end
		product = 1
		(0..3).each do |o|
